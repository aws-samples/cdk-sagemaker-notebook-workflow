...
def lambda_handler(event, context):
    ...
    email_body = ('Hi,<br/><br/>'
                 'Your notebook instance named "' + nb_name + '" is not running as it is in a stopped state.<br/><br/>'
                 'To start your instance confirm <a href="' + api_gw_uri + '/succeed?taskToken=' + urllib.parse.quote(task_token) + '">here</a><br/><br/>'
                 'Else to do keep it stopped click here <a href="' + api_gw_uri + '/fail?taskToken=' + urllib.parse.quote(task_token) + '">here</a>.')
    email_msg = { 
        'Subject': { 
            'Data': 'Confirm start of SageMaker Notebook: ' + nb_name, 
            'Charset': 'UTF-8' 
        },
        'Body': { 
                'Html': {
